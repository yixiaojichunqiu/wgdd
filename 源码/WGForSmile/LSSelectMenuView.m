...
- (void)openShowView{
    ...
    for(UIButton *btn in [nowView subviews])
    {
        LSButton* menubtn=(LSButton*)[self viewWithTag:100+selectIndex ];
        if ([btn.titleLabel.text isEqualToString:menubtn.title]) {
            btn.selected=YES;
        }
    }
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    if ([touch.view isKindOfClass:[UIButton class]]) {
        for(UIButton *btn in [self.nowview subviews])
        {
            btn.selected=NO;
        }
        ...
        btn.selected=YES;
        LSButton* menubtn=(LSButton*)[self viewWithTag:100+selectIndex ];
        [menubtn setTitle:btn.titleLabel.text];
        [NSTimer scheduledTimerWithTimeInterval:0.3 target:self selector:@selector(delay) userInfo:nil repeats:NO];
